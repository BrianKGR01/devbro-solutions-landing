...
## D26 · Conexión real a Supabase y Resend

El humano compartió accesos reales: un servidor MCP de Supabase (comando `claude mcp add`), una cadena de conexión directa a Postgres, la API key de Resend, y una captura confirmando que `devbro.xyz` ya está verificado en Resend (DKIM y SPF verificados, "Domain verified").

**El MCP de Supabase ya conectado en la sesión apunta a otro proyecto.** `list_projects` solo devuelve `ceom-services` (`riertvgnjaujstwyqoom`) — no existe `.mcp.json` en este repo, así que ese servidor está configurado a nivel de usuario y quedó autenticado contra una cuenta/proyecto distinto del `wdbzgvjnkykgvctcwnvf` (devbro-solutions) que pide la instrucción. El comando `claude mcp add --scope project ...` que se compartió agrega un servidor *nuevo*, con su propio paso de autenticación interactiva (`claude /mcp`, que el propio mensaje del humano aclara que hay que correr en una terminal real, no en este agente) — no se pudo completar esa autenticación desde acá, y una vez agregada, una herramienta MCP nueva típicamente no queda disponible hasta reconectar la sesión. En vez de bloquear el trabajo esperando ese reinicio, se resolvió por otra vía (ver abajo). El `claude mcp add` sigue siendo válido para dejarlo configurado de cara a futuras migraciones desde este mismo agente — recomendado correrlo igual.

**Conexión directa: el host `db.*.supabase.co` es IPv6-only.** La cadena de conexión que se compartió (`db.wdbzgvjnkykgvctcwnvf.supabase.co:5432`) solo resuelve a una dirección IPv6 (`nslookup` lo confirma, sin registro `A`) — comportamiento estándar de Supabase para proyectos nuevos, y esta máquina no tiene salida IPv6. Se usó en su lugar el **pooler de conexión (Supavisor)**, que sí resuelve por IPv4: `postgresql://postgres.wdbzgvjnkykgvctcwnvf:[password]@aws-0-sa-east-1.pooler.supabase.com:5432/postgres` — la región (`sa-east-1`) se adivinó por ser la misma que usa el otro proyecto de la cuenta (`ceom-services`) y resultó correcta al primer intento. El cliente `pg` se instaló temporalmente con `--no-save --no-package-lock` (igual que Playwright en fases anteriores) solo para correr esta migración puntual — no queda como dependencia del proyecto.

**Tabla `leads` creada tal cual la especifica `docs/04-engineering.md` §6**, sin modificaciones: mismas columnas, RLS habilitado, **cero políticas públicas** (verificado con una consulta a `pg_policies` después de crearla). Eso significa que la tabla es de escritura exclusiva vía la `service_role key` desde `src/pages/api/contacto.ts` — ningún cliente ni rol anónimo puede escribir ni leer directamente, ni con la clave pública. Se evaluó cambiar a `anon key` + política de RLS que solo permita `INSERT` (un patrón mas común para formularios públicos, y que hubiera evitado depender de una clave que no se puede obtener por API), pero `docs/04-engineering.md` ya especifica explícitamente "sin políticas públicas: solo se escribe desde la ruta API con la service key" — es una decisión ya tomada, no una ambigüedad a resolver en silencio, así que se implementó como está documentado.

**La `service_role key` no se pudo obtener por MCP ni por la cadena de conexión.** Supabase nunca expone esa clave por su API de gestión (por diseño: la herramienta `get_publishable_keys` del MCP solo devuelve claves públicas/anon) y la cadena de conexión a Postgres es una credencial distinta (usuario/contraseña de base de datos, no la clave de servicio tipo JWT que espera `@supabase/supabase-js`). **Sigue pendiente que el humano la copie de Supabase → Project Settings → API → `service_role` secret key.**

**Remitente de Resend actualizado.** `src/pages/api/contacto.ts` dejó de usar `onboarding@resend.dev` (el remitente de prueba que no requiere dominio propio) y pasa a `DevBro Solutions <info@devbro.xyz>`, ya que la captura confirma el dominio verificado. Se reutiliza `info@devbro.xyz` (la misma dirección pública del pie de página, D24) en vez de inventar un alias nuevo tipo `notificaciones@` — no había ninguna instrucción específica sobre qué dirección usar, y este correo es una notificación interna (el `replyTo` ya apunta al correo real del lead, no a este remitente).

**Verificado de punta a punta con un envío real.** Con `RESEND_API_KEY` cargada en un `.env` local (gitignored) y `SUPABASE_SERVICE_KEY` deliberadamente vacía (no disponible todavía), se hizo un POST real a `/api/contacto` con datos de prueba con el nombre `[PRUEBA] Verificacion via curl` para que se distinga de un lead real a simple vista. La respuesta fue `{"ok":true}` en 572ms — dado que sin `SUPABASE_SERVICE_KEY` el guardado en Supabase falla de forma determinística (`guardarEnSupabase` retorna `false` sin tocar la red), un `ok:true` solo es posible si el envío por Resend funcionó. Confirma que la degradación elegante de D19/docs 04 §6 sigue funcionando correctamente además de confirmar Resend en sí.
